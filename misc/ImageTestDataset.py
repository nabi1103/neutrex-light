...
class CustomTestData(TestData):
    def __init__(self, testpath, iscrop=True, crop_size=224, scale=1.25, face_detector='fan',
                 scaling_factor=1.0, max_detection=None, label_path=None):
        self.max_detection = max_detection
        if isinstance(testpath, list):
            self.imagepath_list = testpath
        elif os.path.isdir(testpath):
        ...
        if label_path is not None:
            self.label_dir = pd.read_csv(label_path) ##TODO
        # add_pretrained_deca_to_path()
        # from decalib.datasets import detectors
        if face_detector == 'fan':
            self.face_detector = FAN()
    ...
    def __getitem__(self, index):
        ...
        image = image / 255.
        # label = [self.label_dir.loc[imagename + ".jpg", "img_names"]]
        # label = [self.label_dir.loc[self.label_dir['img_names'] == imagename + ".jpg"]["labels"].values[0]]
        label = imagename
        ...
        if not isinstance(src_pts, list):
            ...
            imagepaths = [imagepath]* dst_images.shape[0]
            res = {'image': torch.tensor(dst_images).float(),
                    'image_name': imagenames,
                    'image_path': imagepaths,
                    'label_path': label
                    }
